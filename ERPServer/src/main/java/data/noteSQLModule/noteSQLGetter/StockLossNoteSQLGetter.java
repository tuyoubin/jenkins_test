...
public class StockLossNoteSQLGetter extends NoteSQLGetter{

	public StockLossNoteSQLGetter(String dataBaseName) {
		super(dataBaseName);
	}
	...
	public List<NotePO> getNoteList() {
		...
		try {
			...
			while(res.next()){
				...
				StockLossNotePO stockLossNotePO = new StockLossNotePO(noteNumber, userID, goodsListItemNum,
						goodsIDList, goodsNameList, stockAmounts, realAmounts, isPassed);		
				list.add(stockLossNotePO);
			}
